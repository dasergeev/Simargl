...
namespace RailTest.Algebra
{
    ...
    public class Vector<T> : ICloneable
    {
        ...
        {
            _Length = length >= 0 ? length : throw new ArgumentOutOfRangeException("length", "Передано отрицательное значение.");
            _Items = new T[length];
        }
        ...
        public static Vector<T> operator + (Vector<T> left, Vector<T> right)
        {
            ...
            if (right is null)
            {
                throw new ArgumentNullException("right", "Передана пустая ссылка.");
            }
        ...
        {
            Vector<T> duplicate = new Vector<T>();
            if (_Length > 0)
            {
