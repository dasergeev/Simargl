...
namespace RailTest.Algebra
{
    ...
    public unsafe sealed class RealVector : Vector
    {
        ...
        public RealVector(params double[] items) :
            this((items ?? throw new ArgumentNullException("items", "Передана пустая ссылка.")).Length)
        {
            int length = Length;
        ...
        public double GetMinProbable(double probability, ProbabilityType type)
        {
            ...
            switch (type)
            {
                ...
                default:
                    throw new ArgumentOutOfRangeException("type", "Значение не содержится в перечислении.");
            }
        }
        ...
        public void Reverse(int index, int count)
        {
            ...
            if (count > 1)
            {
                ...
                {
                    double value = _Items[index + i];
                    _Items[index + i] = _Items[index + count - i - 1];
                    _Items[index + count - i - 1] = value;
                }
            }
        }
        ...
        public RealVector GetSubVector(int index, int length)
        {
            ...
            if (length < 0)
            {
                throw new ArgumentOutOfRangeException("length", "Произошла попытка получить подвектор отрицательной размерности.");
            }
            ...
            RealVector subVector = new RealVector(length);
            if (length > 0)
            {
        ...
        private double Real_ErrorFunction(double argument)
        {
        ...
        void RealSeries_Sort(double* series, int length)
        {
            double* temporal = (double*)MemoryManager.Alloc(length * sizeof(double));
            try
            {
        ...
        public static RealVector operator + (RealVector left, RealVector rigth)
        {
            if (left is null)
            {
                throw new ArgumentNullException("left", "Передана пустая ссылка.");
            }
            if (rigth is null)
            {
                throw new ArgumentNullException("rigth", "Передана пустая ссылка.");
            }
