    ...
    IEnumerable<Channel>
{
    ...
    public Channel this[int index]
    {
        ...
        set
        {
            ...
            {
                //  Передано значение, которое уже содержится коллекции.
                throw Exceptions.ArgumentAlreadyContainedInCollection(nameof(Channel));
            }
    ...
    public void AddRange(IEnumerable<Channel> collection)
    {
        ...
        if (collection.Any(x => x is null))
        {
            ...
            throw Exceptions.ArgumentNullReference(nameof(collection));
        }
        ...
        {
            //  Передано значение, которое уже содержится коллекции.
            throw Exceptions.ArgumentAlreadyContainedInCollection(nameof(collection));
        }
    ...
    public Channel this[string name]
    {
        get
        {
            ...
            //  Поиск канала.
            Channel? channel = _Items.FirstOrDefault(x => x.Name == name);

            //  Проверка найденного канала.
            if (channel is null)
            {
                //  Элемент не содержится в коллекции.
                throw Exceptions.ArgumentNotContainedInCollection(nameof(name));
            }
