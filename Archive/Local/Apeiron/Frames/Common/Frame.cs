...
public class Frame
{
    ...
    public StorageFormat Format
    {
        ...
        set
        {
            ...
            if (Header.Format != value)
            {
                ...
                Header = FrameHeader.Convert(Header, value);
        ...
        try
        {
            ...
            if (numberOfChannels > 0)
            {
                for (int i = 0; i != numberOfChannels; ++i)
                {
                    ...
                    CatmanChannelHeader channelHeader = new()
                    {
                        LocationInDatabase = spreader.ReadInt16()
                    };
                    channelLength = spreader.ReadInt32();
                    Signal signal = new(1, new(channelLength));
    ...
    static void SaveCatmanFrame(Frame frame, Spreader spreader)
    {
        ...
        try
        {
            CatmanFrameHeader frameHeader = (CatmanFrameHeader)FrameHeader.Convert(
                frame.Header, StorageFormat.Catman);
    ...
    {
        TestLabFrameHeader frameHeader = (TestLabFrameHeader)FrameHeader.Convert(
            frame.Header, StorageFormat.TestLab);
