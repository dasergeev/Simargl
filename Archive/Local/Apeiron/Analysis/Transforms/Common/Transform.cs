...
public abstract class Transform
{
    ...
    /// В параметре <paramref name="source"/> передана пустая ссылка.
    /// </exception>
    public void Invoke(Channel source)
    {
        //  Проверка ссылки на исходный объект.
        IsNotNull(source, nameof(source));

        //  Выполнение преобразования.
        InvokeCore(source.Signal, source.Signal);
    }
    ...
    /// В параметре <paramref name="source"/> передана пустая ссылка.
    /// </exception>
    public Channel CloneInvoke(Channel source)
    {
        //  Проверка ссылки на исходный объект.
        IsNotNull(source, nameof(source));

        //  Создание копии исходного объекта.
        Channel target = source.Clone();

        //  Выполнение преобразования.
        InvokeCore(source.Signal, target.Signal);
    ...
    /// В параметре <paramref name="target"/> передана пустая ссылка.
    /// </exception>
    public void Invoke(Channel source, Channel target)
    {
        //  Проверка ссылок.
        IsNotNull(source, nameof(source));
        IsNotNull(target, nameof(target));
    ...
    /// Преобразованный объект.
    /// </param>
    internal protected abstract void InvokeCore([ParameterNoChecks] Signal source, [ParameterNoChecks] Signal target);
}
