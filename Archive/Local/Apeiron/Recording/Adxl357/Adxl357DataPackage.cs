using Apeiron.Cryptography;
using Apeiron.IO;
using Apeiron.Recording.Stm32;
using System.Text;

namespace Apeiron.Recording.Adxl357;

/// <summary>
/// Представляет пакет данных датчика ADXL357.
/// </summary>
public sealed class Adxl357DataPackage
{
    /// <summary>
    ...
    /// </summary>
    public Adxl357DataPackage()
    {
        //  Инициализация массивов синхронных данных.
        _XSignalData = Array.Empty<float>();
        _YSignalData = Array.Empty<float>();
        _ZSignalData = Array.Empty<float>();
    ...
    /// </summary>
    public Adxl357AsyncValueCollection AsyncValues { get; }

    /// <summary>
    /// Возвращает коллекцию сигналов.
    /// </summary>
    public Adxl357SignalCollection Signals { get; }
    ...
    /// </exception>
    public static Adxl357DataPackage Load(Stream stream)
    {
        //  Проверка потока.
        ...
        //  Создание пакета.
        Adxl357DataPackage package = new();
    ...
    public async Task SaveAsync(Stream stream, CancellationToken cancellationToken)
    {
        ...
        using (MemoryStream memoryStream = new(buffer))
        {
            ...
            //  Запись сигнатуры.
            await spreader.WriteInt64Async(_Signature, cancellationToken).ConfigureAwait(false);
    ...
    /// Неверная контрольная сумма пакета данных Adxl.
    /// </exception>
    public static async Task<Adxl357DataPackage> LoadAsync(Stream stream, CancellationToken cancellationToken)
    {
        //  Проверка потока.
        stream = IsReadable(stream, nameof(stream));
