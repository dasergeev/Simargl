using Ably.Designing.Utilities;
using Microsoft.Extensions.Logging;
using Simargl.IO;
using Simargl.Platform.Recorder.FileTransferClient.Main;
using Simargl.Platform.Specialized.FileTransfer;
...
public class ClientRule
{
    ...
    public async Task InvokeAsync(ILogger logger, ClientLogic logic, CancellationToken cancellationToken)
    {
        ...
        ClientConnection connection = IsNotNull(logic.Connection, nameof(connection));

        //  Получение имени домена для подключения.
        string domain = IsNotNull(connection.Domain, nameof(domain));

        //  Получение порта для подключения.
        int port = IsHasValue(connection.Port, nameof(port));

        //  Получение идентификатора.
        int identifier = IsHasValue(connection.Identifier, nameof(identifier));

        //  Получение имени.
        string name = IsNotNull(Name, nameof(name));

        //  Получение пути.
        string path = PathBuilder.Normalize(IsNotNull(Path, nameof(path)));
        ...
        Stream stream, DirectoryInfo directory, CancellationToken cancellationToken)
    {
        ...
        stream = IsNotNull(stream, nameof(stream));

        //  Проверка ссылки на каталог.
        directory = IsNotNull(directory, nameof(directory));
        ...
        Stream stream, FileInfo file, CancellationToken cancellationToken)
    {
        ...
        file = IsNotNull(file, nameof(file));
        ...
        //  Получение задержки.
        int delay = IsHasValue(Delay, nameof(delay));
        ...
        string rootPath = PathBuilder.Normalize(IsNotNull(Path, nameof(Path)));
