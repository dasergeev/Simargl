    ...
    where TPerformer : Performer
{
    ...
    public PerformerService(ILogger<PerformerService<TPerformer>> logger)
    {
        ...
        _Journal = Journal.FromLogger(Check.IsNotNull(logger, nameof(logger)));

        //  Поиск конструктора исполнителя.
        ConstructorInfo? constructor = typeof(TPerformer).GetConstructor(new Type[] { typeof(Journal) });

        //  Проверка констурктора исполнителя.
        if (constructor is null)
        {
            //  Не найден конструктор исполнителя.
            throw Exceptions.OperationNotSupported();
        }
    ...
    public TimeSpan Timeout
    {
        ...
        {
            //  Проверка таймаута.
            Check.IsTimeout(value, nameof(Timeout));
