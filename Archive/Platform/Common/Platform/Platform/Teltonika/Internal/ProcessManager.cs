        ...
        lock (_SyncRoot)
        {
            ...
            foreach (var one in _Interfaces)
            {
                ...
                try
                {
                    ...
                    //  Разбор широты
                    float latitude = float.Parse(Check.IsNotNull(reader.ReadLine(), nameof(reader)), CultureInfo.InvariantCulture);

                    //  Разбор долготы
                    float longitude = float.Parse(Check.IsNotNull(reader.ReadLine(), nameof(reader)), CultureInfo.InvariantCulture);

                    //  Разбор высоты
                    float altitude = float.Parse(Check.IsNotNull(reader.ReadLine(), nameof(reader)), CultureInfo.InvariantCulture);

                    //  Разбор скорости
                    float speed = float.Parse(Check.IsNotNull(reader.ReadLine(), nameof(reader)), CultureInfo.InvariantCulture);

                    //  Чтение времени.
                    string time = Check.IsNotNull(reader.ReadLine(), nameof(reader));

                    //  Разбор точности
                    float accuracy = float.Parse(Check.IsNotNull(reader.ReadLine(), nameof(reader)), CultureInfo.InvariantCulture);

                    //  Разбор количества спутников.
                    int satelite = int.Parse(Check.IsNotNull(reader.ReadLine(), nameof(reader)));
