    ...
    IDisposable
{
    ...
    public GeolocationManager(ModbusManager modbusManager, ModemManager modemManger, ProcessManager processManager, int nmeaPort,int geolocationPort)
    {
        ...
        _ModbusManager = Check.IsNotNull(modbusManager,nameof(modbusManager));

        //  Установка интерфейса Modem
        _ModemManager = Check.IsNotNull(modemManger,nameof(modbusManager));

        //  Установка интерфейса Process
        _ProcessManager = Check.IsNotNull(processManager,nameof(processManager));
        ...
        while (!cancellationToken.IsCancellationRequested)
        {
            try
            {
                if (udpClient.Available > 0)
                {
                    ...
                    //  Чтение префикса.
                    string prefix = Check.IsNotNull(line, nameof(line));
                    ...
                    //  Чтение IMEI
                    var imei = Check.IsNotNull(line, nameof(line));

                    //  Чтение строки.
                    line = await reader.ReadLineAsync().ConfigureAwait(false);

                    //  Чтение ICCID
                    var iccid = Check.IsNotNull(line, nameof(line));

                    //  Чтение строки.
                    line = await reader.ReadLineAsync().ConfigureAwait(false);

                    //  Получение широты
                    float latitude = float.Parse(Check.IsNotNull(line, nameof(line)), CultureInfo.InvariantCulture);

                    //  Чтение строки.
                    line = await reader.ReadLineAsync().ConfigureAwait(false);

                    //  Получение долготы
                    float longitude = float.Parse(Check.IsNotNull(line, nameof(line)), CultureInfo.InvariantCulture);

                    //  Чтение строки.
                    line = await reader.ReadLineAsync().ConfigureAwait(false);

                    //  Получение высоты.
                    float altitude = float.Parse(Check.IsNotNull(line, nameof(line)), CultureInfo.InvariantCulture);

                    //  Чтение строки.
                    line = await reader.ReadLineAsync().ConfigureAwait(false);

                    //  Получение cкорости.
                    float speed = float.Parse(Check.IsNotNull(line, nameof(line)), CultureInfo.InvariantCulture);

                    //  Чтение времени.
                    var time = Check.IsNotNull(await reader.ReadLineAsync().ConfigureAwait(false), nameof(line));

                    //  Чтение строки.
                    line = await reader.ReadLineAsync().ConfigureAwait(false);

                    //  Получение точности.
                    float accuracy = float.Parse(Check.IsNotNull(line, nameof(line)), CultureInfo.InvariantCulture);

                    //  Чтение строки.
                    line = await reader.ReadLineAsync().ConfigureAwait(false);

                    //  Получение количества спутников.
                    int satelite = int.Parse(Check.IsNotNull(line, nameof(line)));
