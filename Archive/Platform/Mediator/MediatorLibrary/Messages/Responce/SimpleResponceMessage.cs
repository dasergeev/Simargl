...
namespace Apeiron.Platform.MediatorLibrary.Messages.Responce;

/// <summary>
/// Представляет простое ответное сообщение от сервера.
/// </summary>
public class SimpleResponceMessage : GeneralMessage
{
    /// <summary>
    ...
    public override async Task<GeneralMessage> LoadPackageAsync(Stream stream, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            var result = new SimpleResponceMessage()
            {
                IdSequence = receiveIdSequence,
                Type = receiveFormat,
                HostId = receiveHostId,
                DataText = receiveText
            };
    ...
    public override async Task SavePackageAsync(Stream stream, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            if (!string.IsNullOrEmpty(DataText))
                await spreader.WriteStringAsync(DataText, cancellationToken).ConfigureAwait(false);
