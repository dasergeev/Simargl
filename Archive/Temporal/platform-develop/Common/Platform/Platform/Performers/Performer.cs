...
namespace Apeiron.Platform.Performers;
...
public abstract class Performer
{
    ...
    {
        //  Установка журнала.
        Journal = Check.IsNotNull(journal, nameof(journal));
    }
    ...
    protected async ValueTask KeepAsync(
        [ParameterNoChecks] AsyncAction action,
        [ParameterNoChecks] int delay, CancellationToken cancellationToken)
    {
    ...
    /// </exception>
    protected async ValueTask KeepAsync(
        [ParameterNoChecks] AsyncAction action, CancellationToken cancellationToken)
    {
    ...
    /// </exception>
    protected async ValueTask KeepAsync(
        [ParameterNoChecks] IEnumerable<AsyncAction> actions,
        [ParameterNoChecks] int delay, CancellationToken cancellationToken)
    {
        //  Проверка токена отмены.
        await Check.IsNotCanceledAsync(cancellationToken).ConfigureAwait(false);
    ...
    /// </exception>
    protected async ValueTask KeepAsync(
        [ParameterNoChecks] IEnumerable<AsyncAction> actions, CancellationToken cancellationToken)
    {
