    ...
    IDisposable
{
    ...
    public SshCommander(string host, int port, string username, string password)
    {
        ...
        _Host = Check.IsNotNull(host, nameof(host));
        
        //  Инициализация порта удаленного узла.
        _Port = port;

        //  Инициализация имени пользователя.
        _Username = Check.IsNotNull(username, nameof(username));

        //  Инициализация пароля пользователя.
        _Password = Check.IsNotNull(password,nameof(password));
    ...
    public void UploadFile(string localPath, string remotePath)
    {
        lock (_SyncRoot)
        {
            ...
            Check.IsNotNull(remotePath, nameof(remotePath));
