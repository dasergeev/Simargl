    ...
    Node
{
    ...
    /// </summary>
    private AccelEth3TConnect _Connect;
    ...
    /// </exception>
    public AdxlDevice(long serialNumber, AccelEth3TConnect connect, Action<Action> invoker) :
        base($"{serialNumber:X8}")
    {
        Invoker = invoker;
    ...
    /// <summary>
    /// Возвращает объект, выполняющий действие в основном потоке.
    /// </summary>
    public Action<Action> Invoker { get; }
    ...
    /// </exception>
    [Browsable(false)]
    public AccelEth3TConnect Connect
    {
        get => _Connect;
        set
        {
            ...
            {
                //  Произошла попытка выполнить недопустимую операцию.
                throw ExceptionsCreator.OperationInvalid();
            }
    ...
    public string AddressSettler
    {
        get => _Address.ToString();
        set => _Address = new(IPAddress.Parse(value));
    }
    ...
    public string MaskSettler
    {
        get => _Mask.ToString();
        set => _Mask = new(IPAddress.Parse(value));
    }
    ...
    public string GatewaySettler
    {
        get => _Gateway.ToString();
        set => _Gateway = new(IPAddress.Parse(value));
    }
    ...
    public string ServerSettler
    {
        get => _Server.ToString();
        set => _Server = new(IPAddress.Parse(value));
    }
    ...
    public async Task UpdateAsync(CancellationToken cancellationToken)
    {
        ...
        //  Чтение типа датчика.
        string sensorType = await Connect.ReadSensorTypeAsync(cancellationToken).ConfigureAwait(false);

        //  Чтение версии прошивки.
        string firmwareVersion = await Connect.ReadFirmwareVersionAsync(cancellationToken).ConfigureAwait(false);

        //  Чтение даты изготовления прошивки.
        string firmwareDate = await Connect.ReadFirmwareDateAsync(cancellationToken).ConfigureAwait(false);

        //  Чтение значения, определяющего использование DHCP.
        bool useDhcp = await Connect.ReadUseDhcpAsync(cancellationToken).ConfigureAwait(false);

        //  Чтение UDP порта для подключения к датчику.
        ushort udpPort = await Connect.ReadUdpPortAsync(cancellationToken).ConfigureAwait(false);

        //  Чтение TCP порта для подключения к серверу.
        ushort tcpPort = await Connect.ReadTcpPortAsync(cancellationToken).ConfigureAwait(false);

        //  Чтение диапазона измерения датчика.
        ushort measuringRange = await Connect.ReadMeasuringRangeAsync(cancellationToken).ConfigureAwait(false);

        //  Чтение значения HighPass фильтра.
        ushort highPassFilter = await Connect.ReadHighPassFilterAsync(cancellationToken).ConfigureAwait(false);

        //  Чтение частоты дискретизации датчика.
        ushort sampling = await Connect.ReadSamplingAsync(cancellationToken).ConfigureAwait(false);

        //  Чтение смещения сигнала по оси Ox.
        float xOffset = await Connect.ReadXOffsetAsync(cancellationToken).ConfigureAwait(false);

        //  Чтение смещения сигнала по оси Oy.
        float yOffset = await Connect.ReadYOffsetAsync(cancellationToken).ConfigureAwait(false);

        //  Чтение смещения сигнала по оси Oz.
        float zOffset = await Connect.ReadZOffsetAsync(cancellationToken).ConfigureAwait(false);

        //  Чтение IP адреса датчика.
        IPv4Address address = await Connect.ReadAddressAsync(cancellationToken).ConfigureAwait(false);

        //  Чтение маски подсети.
        IPv4Address mask = await Connect.ReadMaskAsync(cancellationToken).ConfigureAwait(false);

        //  Чтение шлюза.
        IPv4Address gateway = await Connect.ReadGatewayAsync(cancellationToken).ConfigureAwait(false);

        //  Чтение адреса сервера.
        IPv4Address server = await Connect.ReadServerAsync(cancellationToken).ConfigureAwait(false);

        //  Чтение максимального значения температуры.
        float maxTemperature = await Connect.ReadMaxTemperatureAsync(cancellationToken).ConfigureAwait(false);

        //  Чтение минимального значения температуры.
        float minTemperature = await Connect.ReadMinTemperatureAsync(cancellationToken).ConfigureAwait(false);

        //  Чтение максимального значения напряжения питания.
        float maxVoltage = await Connect.ReadMaxVoltageAsync(cancellationToken).ConfigureAwait(false);

        //  Чтение минимального значения напряжения питания.
        float minVoltage = await Connect.ReadMinVoltageAsync(cancellationToken).ConfigureAwait(false);

        //  Чтение значения самодиагностики акселерометра.
        float diagnosticValue = await Connect.ReadDiagnosticValueAsync(cancellationToken).ConfigureAwait(false);

        //  Чтение информации о сохранённых ошибках.
        uint errorCodes = await Connect.ReadErrorCodesAsync(cancellationToken).ConfigureAwait(false);

        //  Чтение состояния датчика.
        bool enable = await Connect.ReadStateAsync(cancellationToken).ConfigureAwait(false) != 0;
