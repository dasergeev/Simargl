...
public unsafe class RealMatrix : Matrix
{
    ...
    public double this[int row, int column]
    {
        get
        {
            if (row < 0)
            {
                throw new ArgumentOutOfRangeException(nameof(row), "Передано отрицательное значение.");
            }
            else if (row >= RowsCount)
            {
                throw new ArgumentOutOfRangeException(nameof(row), "Передано значение, большее или равное количеству строк.");
            }
            if (column < 0)
            {
                throw new ArgumentOutOfRangeException(nameof(column), "Передано отрицательное значение.");
            }
            else if (column >= ColumnsCount)
            {
                throw new ArgumentOutOfRangeException(nameof(column), "Передано значение, большее или равное количеству столбцов.");
            }
