...
namespace RailTest.Frames
{
    ...
    public class Channel
    {
        ...
        public Channel(string name, string unit, double sampling, double cutoff, int length)
        {
            ...
            if (length < 0)
            {
                throw new ArgumentOutOfRangeException(nameof(length), "Произошла попытка создать канал отрицательной длины.");
            }
        ...
        public Channel(ChannelHeader header, RealVector vector)
        {
            ...
            _Vector = vector ?? throw new ArgumentNullException(nameof(vector), "Передана пустая ссылка.");
        }
        ...
        public RealVector Vector
        {
            ...
            set
            {
                _Vector = value ?? throw new ArgumentNullException(nameof(value), "Передана пустая ссылка.");
            }
        }
        ...
        unsafe public void SetZeroAtStart(double startTime)
        {
            if (startTime < 0)
            {
                throw new ArgumentOutOfRangeException(nameof(startTime), "Время для оценки смещения не может быть отрицательным.");
            }
            lock (SyncRoot)
            {
                if (Length != 0)
        ...
        unsafe static void MovingAverage(double* series, int length, double[] weights, int weightsOffset)
        {
            ...
            if (weights == null)
            {
                throw new ArgumentNullException(nameof(weights), "Передана пустая ссылка.");
            }
            if (weights.Length == 0)
            {
            ...
            double* weights, int weightsCount, int weightsOffset)
        {
            ...
            {
                throw new ArgumentNullException(nameof(targetSeries), "Передан нулевой указатель.");
            }
            if (sourceSeries == null)
            {
                throw new ArgumentNullException(nameof(sourceSeries), "Передан нулевой указатель.");
            }
            if (weights == null && weightsCount != 0)
            {
                throw new ArgumentNullException(nameof(weights), "Передан нулевой указатель.");
            }
            if (weightsOffset >= weightsCount)
            {
                throw new ArgumentOutOfRangeException(nameof(weightsOffset), "Смещение должно быть меньше количества весов.");
            }
