...
#pragma warning disable CS3003 // Тип несовместим с CLS
...
/// </summary>
internal class ForwardedPortDynamic : ForwardedPort
{
    ...
    private bool HandleSocks(IChannelDirectTcpip channel, Socket clientSocket, TimeSpan timeout)
    {
        ...
        try
        {
            ...
            {
                -1 => false,// SOCKS client closed connection
                4 => HandleSocks4(clientSocket, channel, timeout),
                5 => HandleSocks5(clientSocket, channel, timeout),
                _ => throw new NotSupportedException(string.Format(CultureInfo.InvariantCulture, "SOCKS version {0} is not supported.", version)),
            };
        }
