...
using Simargl.Zero.Ssh.Renci.SshNet.NetConf;
using Simargl.Zero.Ssh.Renci.SshNet.Security;
using Simargl.Zero.Ssh.Renci.SshNet.Sftp;
...
internal sealed partial class ServiceFactory : IServiceFactory
{
    ...
    public IConnector CreateConnector(IConnectionInfo connectionInfo, ISocketFactory socketFactory)
    {
        ...
        return connectionInfo.ProxyType switch
        {
            ProxyTypes.None => new DirectConnector(socketFactory),
            ProxyTypes.Socks4 => new Socks4Connector(socketFactory),
            ProxyTypes.Socks5 => new Socks5Connector(socketFactory),
            ProxyTypes.Http => new HttpConnector(socketFactory),
            _ => throw new NotSupportedException(string.Format("ProxyTypes '{0}' is not supported.", connectionInfo.ProxyType)),
        };
    }
