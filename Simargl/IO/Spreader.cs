...
public sealed class Spreader
{
    ...
    {
        //  Создание ядра распределителя данных потока.
        _Core = new(stream);

        //  Создание буфера распределителя данных потока.
        _Buffer = new(_Core);
    ...
    /// </summary>
    public int ReadPosition => _Core.ReadPosition;

    /// <summary>
    /// Возвращает текущую позицию записи.
    /// </summary>
    public int WritePosition => _Core.WritePosition;
    ...
    {
        get => _Buffer.IsBigEndian;
        set => _Buffer.IsBigEndian = value;
    }
    ...
    {
        //  Чтение данных.
        _Core.ReadBytes(buffer, index, count);
    }
    ...
    {
        //  Чтение данных.
        await _Core.ReadBytesAsync(buffer, index, count, cancellationToken).ConfigureAwait(false);
    }
    ...
    {
        //  Запись данных.
        _Core.WriteBytes(buffer, index, count);
    }
    ...
    {
        //  Запись данных.
        await _Core.WriteBytesAsync(buffer, index, count, cancellationToken).ConfigureAwait(false);
    }
    ...
    {
        //  Запись значения.
        await WriteUInt8Async((byte)(value ? 1 : 0), cancellationToken).ConfigureAwait(false);
    }
    ...
    {
        //  Асинхронная запись в поток.
        await WriteCharsAsync([value], cancellationToken).ConfigureAwait(false);
    }
    ...
    /// Записываемое значение.
    /// </param>
    private void Write7BitEncodedInt([NoVerify] int value)
    {
    ...
    /// Операция отменена.
    /// </exception>
    private async Task Write7BitEncodedIntAsync([NoVerify] int value, CancellationToken cancellationToken)
    {
        //  Проверка токена отмены.
        await IsNotCancelledAsync(cancellationToken).ConfigureAwait(false);
