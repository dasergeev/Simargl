...
/// </summary>
public sealed class ReadMultipleHoldingRegisterCommand :
    ModbusTcpHeader
    //IStreamObject<ReadMultipleHoldingRegisterCommand>,
    //IAsyncStreamObject<ReadMultipleHoldingRegisterCommand>
{
    /// <summary>
    ...
    /// </summary>
    public ReadMultipleHoldingRegisterCommand()
    {
    ...
    /// </exception>
    public ReadMultipleHoldingRegisterCommand(int transactionIdentifier, byte address, int start, int count)
        : base(transactionIdentifier, address, CodeCommand, 6)
    {
        // Проверка адреса.
        IsNotLarger(start, ushort.MaxValue, nameof(start));

        //  Проверка адреса.
        IsNotLess(start, ushort.MinValue, nameof(start));

        // Проверка количества регистров.
        IsNotLarger(count, MaximumRegisterCount, nameof(count));

        // Проверка количества регистров.
        IsNotLess(count, byte.MinValue, nameof(count));
    ...
    /// </exception>
    public new ReadMultipleHoldingRegisterCommand Load(Stream stream)
    {
    ...
    /// Получено значение <see cref="ModbusTcpHeader.Length"/> вне допустимого диапозона.
    /// </exception>
    public new async Task<ReadMultipleHoldingRegisterCommand> LoadAsync(Stream stream, CancellationToken cancellationToken)
    {
        //  Чтение базового класса.
