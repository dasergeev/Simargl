...
public abstract class ModbusTcpHeader 
{
    ...
    protected ModbusTcpHeader(int transactionIdentifier, byte address, byte code, int length)
    {
        ...
        IsNotLarger(transactionIdentifier,ushort.MaxValue,nameof(transactionIdentifier));

        //  Проверка идентификатора.
        IsNotLess(transactionIdentifier,ushort.MinValue,nameof(transactionIdentifier));

        // Проверка длинны.
        IsNotLarger(length, byte.MaxValue + 2, nameof(length));

        //  Проверка длинны.
        IsNotLess(length, byte.MinValue, nameof(length));
    ...
    internal ModbusTcpHeader Load(Stream stream)
    {
        ...
        {
            throw ExceptionsCreator.StreamInvalidFormat();
        }
