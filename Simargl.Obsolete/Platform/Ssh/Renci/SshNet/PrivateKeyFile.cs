...
using Simargl.Zero.Ssh.Renci.SshNet.Security.Cryptography.Ciphers;
using Simargl.Zero.Ssh.Renci.SshNet.Security.Cryptography.Ciphers.Modes;
using Simargl.Zero.Ssh.Renci.SshNet.Security.Cryptography.Ciphers.Paddings;
...
/// </para>
/// </remarks>
internal class PrivateKeyFile : IPrivateKeySource, IDisposable
{
    ...
    private void Open(Stream privateKey, string passPhrase)
    {
        ...
        if (!string.IsNullOrEmpty(cipherName) && !string.IsNullOrEmpty(salt))
        {
            ...
            CipherInfo cipher = cipherName switch
            {
                "DES-EDE3-CBC" => new CipherInfo(192, (key, iv) => new TripleDesCipher(key, new CbcCipherMode(iv), new PKCS7Padding())),
                "DES-EDE3-CFB" => new CipherInfo(192, (key, iv) => new TripleDesCipher(key, new CfbCipherMode(iv), new PKCS7Padding())),
                "DES-CBC" => new CipherInfo(64, (key, iv) => new DesCipher(key, new CbcCipherMode(iv), new PKCS7Padding())),
                "AES-128-CBC" => new CipherInfo(128, (key, iv) => new AesCipher(key, iv, AesCipherMode.CBC, pkcs7Padding: true)),
                "AES-192-CBC" => new CipherInfo(192, (key, iv) => new AesCipher(key, iv, AesCipherMode.CBC, pkcs7Padding: true)),
                "AES-256-CBC" => new CipherInfo(256, (key, iv) => new AesCipher(key, iv, AesCipherMode.CBC, pkcs7Padding: true)),
                _ => throw new SshException(string.Format(CultureInfo.InvariantCulture, "Private key cipher \"{0}\" is not supported.", cipherName)),
            };
    ...
    private static Key ParseOpenSshV1Key(byte[] keyFileData, string passPhrase)
    {
        ...
        if (cipherName != "none")
        {
            ...
            AesCipher cipher = cipherName switch
            {
                "aes256-cbc" => new AesCipher(key, iv, AesCipherMode.CBC, pkcs7Padding: false),
                "aes256-ctr" => new AesCipher(key, iv, AesCipherMode.CTR, pkcs7Padding: false),
                _ => throw new SshException("Cipher '" + cipherName + "' is not supported for an OpenSSH key."),
            };
            try
            {
    ...
    private sealed class SshDataReader : SshData
    {
        ...
        public BigInteger ReadBigIntWithBits()
        {
            ...
            return new BigInteger(bytesArray.ReverseBytes());
        }

        public BigInteger ReadBignum()
        {
            return new BigInteger(ReadBignum2().ReverseBytes());
        }
