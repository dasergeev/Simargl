...
/// </summary>
internal class DsaDigitalSignature : DigitalSignature, IDisposable
{
    ...
    public override bool Verify(byte[] input, byte[] signature)
    {
        ...
        var hm = new BigInteger(hashInput.ReverseBytes().Concat(new byte[] { 0 }));
    ...
    public override byte[] Sign(byte[] input)
    {
        ...
        var m = new BigInteger(hashInput.ReverseBytes().Concat(new byte[] { 0 }));
        ...
        var rBytes = r.ToByteArray().ReverseBytes().TrimLeadingZeros();
        Array.Copy(rBytes, 0, signature, 20 - rBytes.Length, rBytes.Length);

        // issue #1918: pad part with zero's on the left if length is less than 20
        var sBytes = s.ToByteArray().ReverseBytes().TrimLeadingZeros();
