...
/// </summary>
internal class EcdsaKey : Key, IDisposable
{
...
#pragma warning restore IDE0010 // Add missing cases
            ...
            qx = parameter.Q.X;
            qy = parameter.Q.Y;
            byte[] curve = parameter.Curve.Oid.FriendlyName switch
            {
                "ECDSA_P256" or "nistP256" => Encoding.ASCII.GetBytes("nistp256"),
                "ECDSA_P384" or "nistP384" => Encoding.ASCII.GetBytes("nistp384"),
                "ECDSA_P521" or "nistP521" => Encoding.ASCII.GetBytes("nistp521"),
                _ => throw new SshException("Unexpected Curve Name: " + parameter.Curve.Oid.FriendlyName),
            };
#endif
            ...
            return new[] { new BigInteger(curve.ReverseBytes()), new BigInteger(q.ReverseBytes()) };
        }
    }
    ...
    public EcdsaKey(SshKeyData publicKeyData)
    {
        ...
        var curve_s = Encoding.ASCII.GetString(publicKeyData.Keys[0].ToByteArray().ReverseBytes());
        var curve_oid = GetCurveOid(curve_s);

        var publickey = publicKeyData.Keys[1].ToByteArray().ReverseBytes();
    ...
    {
        return curve_s.ToUpperInvariant() switch
        {
            "NISTP256" => ECDSA_P256_OID_VALUE,
            "NISTP384" => ECDSA_P384_OID_VALUE,
            "NISTP521" => ECDSA_P521_OID_VALUE,
