...
using Simargl.Zero.Ssh.Renci.SshNet.Security.Org.BouncyCastle.Crypto.Agreement;
using Simargl.Zero.Ssh.Renci.SshNet.Security.Org.BouncyCastle.Crypto.Generators;
...
using Simargl.Zero.Ssh.Renci.SshNet.Security.Org.BouncyCastle.Security;
...
internal abstract class KeyExchangeECDH : KeyExchangeEC
{
    ...
    private void HandleServerEcdhReply(byte[] hostKey, byte[] serverExchangeValue, byte[] signature)
    {
        ...
        SharedKey = k1.ToByteArray().ToBigInteger2().ToByteArray().ReverseBytes();
    }
}
