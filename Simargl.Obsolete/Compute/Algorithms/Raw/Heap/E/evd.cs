...
public class evd
{
    ...
    * when problem matrix is sparse (and/or is not known explicitly, i.e. only
    ...
    3. User activates out-of-core mode of  the  solver  and  repeatedly  calls
       ...
       >     eigsubspaceoocgetrequestinfo(state, out RequestType, out M)
       >     eigsubspaceoocgetrequestdata(state, out X)
       >     [calculate  Y=A*X, with X=R^NxM]
       >     eigsubspaceoocsendresult(state, in Y)
       > eigsubspaceoocstop(state, out W, out Z, out Report)
    ...
    *************************************************************************/
    public static void eigsubspacecreate(int n,
        ...
        xparams _params)
    {
        ap.assert(n > 0, "EigSubspaceCreate: N<=0");
        ap.assert(k > 0, "EigSubspaceCreate: K<=0");
        ap.assert(k <= n, "EigSubspaceCreate: K>N");
    ...
    *************************************************************************/
    public static void eigsubspacesetcond(eigsubspacestate state,
        ...
        xparams _params)
    {
        ap.assert(!state.running, "EigSubspaceSetCond: solver is already running");
        ap.assert(math.isfinite(eps) && (double)(eps) >= (double)(0), "EigSubspaceSetCond: Eps<0 or NAN/INF");
        ap.assert(maxits >= 0, "EigSubspaceSetCond: MaxIts<0");
    ...
    *************************************************************************/
    public static void eigsubspacesetwarmstart(eigsubspacestate state,
        ...
        xparams _params)
    {
        ap.assert(!state.running, "EigSubspaceSetWarmStart: solver is already running");
    ...
    *************************************************************************/
    public static void eigsubspaceoocstart(eigsubspacestate state,
        ...
        xparams _params)
    {
        ap.assert(!state.running, "EigSubspaceStart: solver is already running");
        ap.assert(mtype == 0, "EigSubspaceStart: incorrect mtype parameter");
    ...
    *************************************************************************/
        ...
        xparams _params)
    {
        ...
        ap.assert(state.running, "EigSubspaceContinue: solver is not running");
    ...
    *************************************************************************/
        ...
        xparams _params)
    {
        ...
        ap.assert(state.running, "EigSubspaceOOCGetRequestInfo: solver is not running");
    ...
    *************************************************************************/
        ...
        xparams _params)
    {
        ...
        ap.assert(!state.running, "EigSubspaceStop: solver is still running");
        n = state.n;
        k = state.k;
        w = new double[k];
        z = new double[n, k];
    ...
    *************************************************************************/
        ...
        xparams _params)
    {
        ...
        ap.assert(!state.running, "EigSubspaceSolveDenseS: solver is still running");
        n = state.n;
        ...
        while (eigsubspaceiteration(state, _params))
        {
            ...
            //
            ap.assert(state.requesttype == 0, "EigSubspaceSolveDense: integrity check failed");
            ap.assert(state.requestsize > 0, "EigSubspaceSolveDense: integrity check failed");
            m = state.requestsize;
    ...
    *************************************************************************/
        ...
        xparams _params)
    {
        ...
        ap.assert(!state.running, "EigSubspaceSolveSparseS: solver is still running");
        n = state.n;
        state.matrixtype = 0;
        state.rstate.ia = new int[7 + 1];
        state.rstate.ra = new double[1 + 1];
    ...
    *************************************************************************/
    ...
    lbl_0:
        ...
        {
            ap.assert(state.matrixtype == 0, "integrity check failed");
            apserv.rmatrixsetlengthatleast(ref state.r, nwork, nwork, _params);
            ...
            {
                ap.assert(false, "EigSubspace: direct eigensolver failed to converge");
            }
            for (j = 0; j <= nwork - 1; j++)
            {
    ...
    lbl_2:
        ...
        //
        ap.assert(state.matrixtype == 0, "integrity check failed");
        ap.assert(state.eigenvectorsneeded == 1);
        ablas.rmatrixgemm(nwork, nwork, n, 1.0, state.qcur, 0, 0, 0, state.ax, 0, 0, 0, 0.0, state.r, 0, 0, _params);
        ...
        for (i = nwork - 1; i >= nwork - k; i--)
        {
            for (i1 = 0; i1 <= nwork - 1; i1++)
            {
                ...
                {
                    ap.assert(cnt < k, "EigSubspace: integrity check failed");
    ...
    *************************************************************************/
        ...
        xparams _params)
    {
        ...
        ap.assert(n >= 1, "SMatrixTDEVD: N<=0");
        ap.assert(zneeded >= 0 && zneeded <= 3, "SMatrixTDEVD: incorrect ZNeeded");
        result = false;
    ...
    *************************************************************************/
        ...
        xparams _params)
    {
        ...
        {
            wastranspose = true;
            RawBlas.inplacetranspose(ref z, 1, n, 1, n, ref wtemp, _params);
        }
        if (zneeded == 2)
        {
            wastranspose = true;
            ...
            for (j = j1; j <= m; j++)
            {
                ...
                else
                {
                    ...
                    do
                    {
                        ...
                        //
                        jmax = RawBlas.vectoridxabsmax(work1, 1, blksiz, _params);
                    ...
                    //
                    scl = 1 / RawBlas.vectornorm2(work1, 1, blksiz, _params);
                    jmax = RawBlas.vectoridxabsmax(work1, 1, blksiz, _params);
                ...
                if (!skipflag)
                {
                    ...
                    if (ip == 0)
                    {
                        ...
                        if (!over)
                        {
                            ...
                            ii = RawBlas.columnidxabsmax(vr, 1, ki, iis, _params);
                        ...
                        else
                        {
                            if (ki > 1)
                            {
                                ...
                                RawBlas.matrixvectormultiply(vr, 1, n, 1, ki - 1, false, work, 1 + n, ki - 1 + n, 1.0, ref temp, 1, n, work[ki + n], _params);
                                for (i_ = 1; i_ <= n; i_++)
                                {
                                    vr[i_, ki] = temp[i_];
                                }
                            }
                            ii = RawBlas.columnidxabsmax(vr, 1, n, ki, _params);
                    ...
                    else
                    {
                        ...
                        else
                        {
                            if (ki > 2)
                            {
                                ...
                                RawBlas.matrixvectormultiply(vr, 1, n, 1, ki - 2, false, work, 1 + n, ki - 2 + n, 1.0, ref temp, 1, n, work[ki - 1 + n], _params);
                                for (i_ = 1; i_ <= n; i_++)
                                {
                                    vr[i_, ki - 1] = temp[i_];
                                }
                                ...
                                RawBlas.matrixvectormultiply(vr, 1, n, 1, ki - 2, false, work, 1 + n2, ki - 2 + n2, 1.0, ref temp, 1, n, work[ki + n2], _params);
                                for (i_ = 1; i_ <= n; i_++)
                                {
                                    vr[i_, ki] = temp[i_];
                                }
                ...
                if (!skipflag)
                {
                    ...
                    if (ip == 0)
                    {
                        ...
                        if (!over)
                        {
                            ...
                            ii = RawBlas.columnidxabsmax(vl, ki, n, iis, _params);
                        ...
                        else
                        {
                            if (ki < n)
                            {
                                ...
                                RawBlas.matrixvectormultiply(vl, 1, n, ki + 1, n, false, work, ki + 1 + n, n + n, 1.0, ref temp, 1, n, work[ki + n], _params);
                                for (i_ = 1; i_ <= n; i_++)
                                {
                                    vl[i_, ki] = temp[i_];
                                }
                            }
                            ii = RawBlas.columnidxabsmax(vl, 1, n, ki, _params);
                    ...
                    else
                    {
                        ...
                        else
                        {
                            if (ki < n - 1)
                            {
                                ...
                                RawBlas.matrixvectormultiply(vl, 1, n, ki + 2, n, false, work, ki + 2 + n, n + n, 1.0, ref temp, 1, n, work[ki + n], _params);
                                for (i_ = 1; i_ <= n; i_++)
                                {
                                    vl[i_, ki] = temp[i_];
                                }
                                ...
                                RawBlas.matrixvectormultiply(vl, 1, n, ki + 2, n, false, work, ki + 2 + n2, n + n2, 1.0, ref temp, 1, n, work[ki + 1 + n2], _params);
                                for (i_ = 1; i_ <= n; i_++)
                                {
                                    vl[i_, ki + 1] = temp[i_];
                                }
