        ...
        xparams _params)
    {
        ...
        if (problemtype == 1)
        {
            ...
            for (j = 1; j <= n; j++)
            {
                ...
                if (isuppera)
                {
                    RawBlas.matrixvectormultiply(a, 0, j - 1, j, n - 1, true, w1, 1, j, 1.0, ref w2, j + 1, n, 0.0, _params);
                }
                else
                {
                    RawBlas.matrixvectormultiply(a, j, n - 1, 0, j - 1, false, w1, 1, j, 1.0, ref w2, j + 1, n, 0.0, _params);
                }
        ...
        if (problemtype == 2 || problemtype == 3)
        {
            ...
            for (j = 1; j <= n; j++)
            {
                ...
                if (isuppera)
                {
                    RawBlas.matrixvectormultiply(a, 0, j - 2, j - 1, n - 1, false, w1, j, n, 1.0, ref w2, 1, j - 1, 0.0, _params);
                }
                else
                {
                    RawBlas.matrixvectormultiply(a, j - 1, n - 1, 0, j - 2, true, w1, j, n, 1.0, ref w2, 1, j - 1, 0.0, _params);
                }
