...
public class ChannelHeader
{
    ...
    internal ChannelHeader(StorageFormat format, string name, string unit, double sampling, double cutoff)
    {
        ...
        if (sampling < 0)
        {
            throw new ArgumentOutOfRangeException(nameof(sampling), "Произошла попытка задать отрицательную частоту дискретизации.");
        }
        _Sampling = sampling;
        _Cutoff = cutoff;
    }
    ...
    public double Sampling
    {
        ...
        set
        {
            if (value < 0)
            {
                throw new ArgumentOutOfRangeException(nameof(value), "Произошла попытка задать отрицательную частоту дискретизации.");
            }
            if (_Sampling != value)
            {
                _Sampling = value;
    ...
    internal virtual ChannelHeader Convert(StorageFormat format)
    {
        ...
        switch (format)
        {
            ...
            case StorageFormat.TestLab:
                {
                    TestLabChannelHeader header = new()
                    {
                        Name = Name,
                        Unit = Unit,
                        Sampling = Sampling,
            ...
            case StorageFormat.Mera:
                throw new Exception();
            default:
                throw new ArgumentOutOfRangeException(nameof(format), "Произошла попытка преобразовать заголовок канала в неизвестный формат.");
        }
    }
