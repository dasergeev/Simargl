    ...
    IDisposable
{
    ...
    public Matrix(int itemSize, int rowsCount, int columnsCount)
    {
        if (itemSize <= 0)
        {
            throw new ArgumentOutOfRangeException(nameof(itemSize), "Передано отрицательное или равное нулю значение.");
        }
        if (rowsCount < 0)
        {
            throw new ArgumentOutOfRangeException(nameof(rowsCount), "Передано отрицательное значение.");
        }
        if (columnsCount < 0)
        {
            throw new ArgumentOutOfRangeException(nameof(columnsCount), "Передано отрицательное значение.");
        }
        RowsCount = rowsCount;
