...
hmWebHelp.tocNav = new function() {
	...
	var dobread = function(args) {
			var bakedbread = " ",
				tempslice = [],
				stop = "",
				nobread = " \<span\>&raquo; Нет разделов более высокого уровня &laquo;\<\/span\>",
				notoc = " \<span\>&raquo; Справочная информация &laquo;\<\/span\>";
	...
	var donavlinks = function(args){
		...
		var $prev = $("a#topicnavlinkprevious"),
			...
			if (args.phf == "none") {
				$prev.attr("class","topicnavlink disabled").attr("title","Это первый раздел").removeAttr("href").removeAttr("data-bs").removeAttr("data-ac").off("click").off(hmBrowser.touchstart);
			} else {
				$prev.attr("class","topicnavlink nav").attr("title","К предыдущему разделу").attr("href",args.phf).attr("data-bs",args.pbs).attr("data-ac",args.pac);
			}
			
			if (args.hhf == "none") {
				$top.attr("class","topicnavlink disabled").attr("title","Нет разделов более высокого уровня").removeAttr("href").removeAttr("data-bs").removeAttr("data-ac").off("click").off(hmBrowser.touchstart);
			} else {
				$top.attr("class","topicnavlink nav").attr("title","На уровень выше").attr("href",args.hhf).attr("data-bs",args.hbs).attr("data-ac",args.hac);
			}
			
			if (args.nhf == "none") {
				$next.attr("class","topicnavlink disabled").attr("title","Это последний раздел").removeAttr("href").removeAttr("data-bs").removeAttr("data-ac").off("click").off(hmBrowser.touchstart);
			} else {
				$next.attr("class","topicnavlink nav").attr("title","К следующему разделу").attr("href",args.nhf).attr("data-bs",args.nbs).attr("data-ac",args.nac);
			}
...
hmWebHelp.hmTopicPageInit = function() {
	...
	if ($("span[class*='_atoc']").length >= parseInt("1",10) && !hmDevice.phone) {
		...
		return function(animOff) {
			...
			if (headerOn) {
				...
				if (hmDevice.desktop) {
					...
					$("li#showhide_pageheader a").first().attr("title","Показать заголовок страницы");
					$("li#showhide_pageheader span").first().html("Показать заголовок страницы");
					}
				inProgress = false;
			...
			hmpage.$headerbox.slideDown((animate ? "400" : "0"), function(){
				...
				if (hmDevice.desktop) {
					...
					$("li#showhide_pageheader a").first().attr("title","Скрыть заголовок страницы");
					$("li#showhide_pageheader span").first().html("Скрыть заголовок страницы");
					}
...
function hmLoadTopic(topicObj) {
	...
	//$("p#ptopic_breadcrumbs").html(" " +(topicObj.hmBreadCrumbs !== "" ? topicObj.hmBreadCrumbs : " \<span\>&raquo; Нет разделов более высокого уровня" + " &laquo;\<\/span\>"));
