...
/// Сердце приложения.
/// </param>
public sealed class RS485RecorderService(
    ILogger<RS485RecorderService> logger, Heart heart) :
    Service<RS485RecorderService>(logger, heart)
{
    /// <summary>
    ...
    protected override sealed async Task InvokeAsync(CancellationToken cancellationToken)
    {
        ...
        DataRecorder recorder = new(
            delegate (DataReceiveResult data)
            {
                ...
                string path = Path.Combine(
                    ...
                    $"{time:yyyy-MM-dd-HH}",
                    "RS485",
                    $"RS485-{time:yyyy-MM-dd-HH-mm}.rs485");
        ...
        IPEndPoint localEndPoint = new(IPAddress.Any, 8501);
        ...
        void received(object? sender, DataReceiverReceivedEventArgs e)
        {
            ...
            if (e.Data is UdpDataReceiveResult data)
            {
                ...
                //  Регистрация данных.
                Heart.Measurer.AddRS485Data(data);
            }
        }
